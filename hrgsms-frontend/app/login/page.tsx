...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try { const res = await apiClient.post("/auth/login", { username, password });
      saveToken(res.data.access_token); if (res.data.role) saveRole(res.data.role); router.push("/dashboard");
  ...
  return (
    <div className="grid cols-2">
      ...
      <div className="card">
        <h2>Why HRGSMS?</h2>
        <ul><li>Quick reservations, accurate billing</li><li>Role-based workflows</li><li>Insightful reports</li></ul>
      </div>
    </div>
  );
}
